    ...
    <span style={{ display:"inline-flex", gap:5, alignItems:"center", padding:"2px 0" }}>
      {[0,1,2].map(i => (
        <span key={i} style={{ width:7, height:7, borderRadius:"50%", background:color, display:"inline-block",
          animation:`pulse 1.2s ${i*.22}s infinite ease-in-out` }} />
      ))}
    </span>
  );
}
...
function Spinner({ color }) {
  return <span style={{ display:"inline-block", width:13, height:13, border:`2px solid ${color}30`,
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.75)",
      ...
      <div onClick={e => e.stopPropagation()} style={{ background:"#16162A", border:"1px solid #2A2A40",
        borderRadius:16, padding:28, width:440, maxWidth:"94vw",
        boxShadow:"0 24px 80px rgba(0,0,0,.6)", animation:"fadeUp .2s ease" }}>

        <div style={{ marginBottom:22 }}>
          <p style={{ fontFamily:"'Syne',sans-serif", fontSize:15, fontWeight:700,
            color:"#E2E2F0", letterSpacing:".04em" }}>Configuración de API Keys</p>
          <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:12, color:"#6B6B90", marginTop:4 }}>
        ...
        {[
          ...
          <div key={key} style={{ marginBottom:16 }}>
            ...
            <input className="config-input" type={type} value={form[key] || ""}
...
export default function OpenClaw() {
  const [config,      setConfig]     = useState({ nexusKey:"", sheetsKey:"", stratySheetId:"" });
  ...
  const callSheets = async (name, inp, sheetId, sheetName) => {
    ...
    if (!sheetId)          return { error:"Falta el Sheet ID — configúralo en ⚙" };
    ...
    try {
      ...
      if (name === "append_payment") {
        ...
        if (!r.ok) return { error:"Escritura requiere OAuth además de API key. Pago para registrar manualmente: " + JSON.stringify(row) };
  ...
  const send = async () => {
    ...
    const history   = histories[active.id] || [];
    ...
    try {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        if (toolUse.length) {
          ...
          finalText = textBlocks.map(b=>b.text).join("") || data.content?.[0]?.text || "Sin respuesta.";
          break;
        }
      }
      ...
      <div style={{ fontFamily:"'DM Sans',sans-serif", background:"var(--bg)", minHeight:"100vh",
        ...
        <header style={{ height:52, display:"flex", alignItems:"center", padding:"0 20px",
          ...
          <span style={{ fontFamily:"'Syne',sans-serif", fontSize:18, fontWeight:800,
            color:"var(--amber)", letterSpacing:".06em" }}>Asistentes Bruno</span>
          <span style={{ fontSize:11, color:"var(--faint)", fontFamily:"'Syne',sans-serif",
            fontWeight:600, letterSpacing:".12em", marginLeft:2 }}>v2</span>
          <div style={{ marginLeft:"auto", display:"flex", gap:8, alignItems:"center" }}>
            <button onClick={() => setShowConfig(true)}
              style={{ display:"flex", alignItems:"center", gap:6, background: configured ? "var(--amber-dim)" : "none",
                border:`1px solid ${configured ? "var(--amber)" : "var(--border2)"}`,
                borderRadius:8, padding:"5px 12px", cursor:"pointer",
                fontFamily:"'Syne',sans-serif", fontSize:10, fontWeight:700,
                color: configured ? "var(--amber)" : "var(--muted)", letterSpacing:".08em" }}>
        ...
        <div style={{ display:"flex", flex:1, overflow:"hidden", height:"calc(100vh - 52px)" }}>
          ...
          {/* ── SIDEBAR ── */}
          <aside style={{ width:210, background:"var(--surface)", borderRight:"1px solid var(--border)",
            display:"flex", flexDirection:"column", flexShrink:0, overflowY:"auto" }}>
            <div style={{ padding:"14px 16px 8px",
              fontFamily:"'Syne',sans-serif", fontSize:9, fontWeight:700,
              letterSpacing:".15em", color:"var(--faint)", textTransform:"uppercase" }}>
              Agentes
            </div>
            {AGENTS.map((a, i) => {
              const isActive = active?.id === a.id;
              const hasHistory = (histories[a.id]||[]).length > 0;
              return (
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontFamily:"'Syne',sans-serif", fontSize:12, fontWeight:700,
                        color: isActive ? a.color : "var(--text)", letterSpacing:".04em",
          ...
          <main style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
            {!active ? (
              ...
              <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center",
                ...
                <p style={{ fontSize:13, color:"var(--faint)", opacity:.5 }}>
                  para comenzar una conversación
                </p>
                {!configured && (
                  <button onClick={() => setShowConfig(true)}
                    style={{ marginTop:8, background:"none", border:"1px solid #F5A62340",
                      borderRadius:10, padding:"10px 22px", color:"#F5A62370",
              ...
              <>
                ...
                <div style={{ padding:"12px 22px", borderBottom:"1px solid var(--border)",
                  ...
                  <div>
                    <p style={{ fontFamily:"'Syne',sans-serif", fontSize:14, fontWeight:700,
                      color:active.color, letterSpacing:".05em" }}>{active.name}</p>
                    <p style={{ fontSize:11, color:"var(--muted)", marginTop:1 }}>{active.role}</p>
                  </div>
                  <div className="tag" style={{ background:`${active.color}18`, border:`1px solid ${active.color}30`,
                    color:active.color, marginLeft:4 }}>
                    {active.badge}
                  </div>
                  {msgs.length > 0 && (
                    <button onClick={() => setHistories(p => ({ ...p, [active.id]:[] }))}
                      style={{ marginLeft:"auto", background:"none", border:"1px solid var(--border2)",
                        borderRadius:7, padding:"4px 12px", color:"var(--muted)",
                ...
                <div style={{ flex:1, overflowY:"auto", padding:"24px 22px",
                  ...
                  {msgs.length === 0 && (
                    <div className="bubble" style={{ alignSelf:"flex-start",
                      background:active.bg, border:`1px solid ${active.color}25`,
                      borderRadius:"4px 14px 14px 14px", padding:"12px 16px",
                      animation:"fadeUp .3s ease" }}>
                      <p style={{ fontSize:11, fontFamily:"'Syne',sans-serif", fontWeight:600,
                        color:active.color, letterSpacing:".06em", marginBottom:6 }}>{active.name}</p>
                    ...
                    return (
                      <div key={i} className="bubble"
                        style={{ alignSelf: isUser ? "flex-end" : "flex-start",
                          background: isUser ? "#1C1C2E" : active.bg,
                          border: `1px solid ${isUser ? "var(--border2)" : `${active.color}25`}`,
                          borderRadius: isUser ? "14px 4px 14px 14px" : "4px 14px 14px 14px",
                          padding:"12px 16px" }}>
                        <p style={{ fontSize:11, fontFamily:"'Syne',sans-serif", fontWeight:600,
                          color: isUser ? "var(--muted)" : active.color,
                          letterSpacing:".06em", marginBottom:6 }}>
                          {isUser ? "Tú" : active.name}
                        </p>
                        <p style={{ color:"var(--text)", whiteSpace:"pre-wrap", wordBreak:"break-word", opacity:.9 }}>
                          {m.content}
                        </p>
                      </div>
                    );
                  ...
                  {loading && (
                    <div className="bubble" style={{ alignSelf:"flex-start",
                      ...
                      borderRadius:"4px 14px 14px 14px", padding:"12px 16px" }}>
                      <p style={{ fontSize:11, fontFamily:"'Syne',sans-serif", fontWeight:600,
                        color:active.color, letterSpacing:".06em", marginBottom:8 }}>{active.name}</p>
                      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                        <Dots color={active.color} />
                        {status && (
                          <span style={{ fontSize:11, color:"var(--muted)", display:"flex",
                            alignItems:"center", gap:6 }}>
                            <Spinner color={active.color} /> {status}
                          </span>
                        )}
                      </div>
                    </div>
                ...
                <div style={{ padding:"14px 22px", borderTop:"1px solid var(--border)",
                  ...
                  <div style={{ display:"flex", gap:10, alignItems:"flex-end",
                    background:"#0C0C18", border:"1px solid var(--border2)",
                    ...
                    <textarea
                      ...
                      style={{ flex:1, background:"none", border:"none", outline:"none",
                        ...
                        lineHeight:1.6, minHeight:26, maxHeight:120, resize:"none",
                        opacity: loading ? .4 : 1 }}
                    />
                    <button className="send-btn" onClick={send}
                      disabled={!input.trim() || loading}
                      style={{ background: !input.trim()||loading ? "var(--faint)" : active.color,
                        color: !input.trim()||loading ? "var(--muted)" : "#0C0C14",
                        opacity: !input.trim()||loading ? .5 : 1 }}>
                      ↑
                    </button>
                  </div>
                  <p style={{ fontSize:10, color:"var(--faint)", marginTop:7, letterSpacing:".04em" }}>
